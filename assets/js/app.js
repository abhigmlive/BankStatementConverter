...
const state = {
  ...
  lastTables: [],
  lastLabel: "",
  cloud: null,            // Firebase API (null = cloud features unavailable)
...
function showResults(tables, label, keepScroll) {
  ...
  const save = $("#save-cloud");
  if (save) save.hidden = !state.user || !tables.length;
...
function updateAuthUI() {
  ...
  const available = !!state.cloud;
  // Sign-in button only when cloud is available and nobody is signed in.
  $("#signin-btn").hidden = !available || signedIn;
  $("#user-box").hidden = !signedIn;
  if (signedIn) $("#user-email").textContent = state.user.email || "Signed in";
  // The Save button shows in results only when signed in.
  const save = $("#save-cloud");
  if (save) save.hidden = !signedIn || !state.lastTables.length;
}

function openLogin(mode = "signin") {
  if (!state.cloud) return toast("Sign-in isn't available right now.", true);
  setLoginMode(mode);
...
function setLoginMode(mode) {
  ...
  $("#login-title").textContent = signup ? "Create account" : "Sign in";
  $("#login-sub").textContent = signup
    ? "Save your conversions and open them on any device."
    : "Welcome back — sign in to your saved conversions.";
  $("#login-submit").textContent = signup ? "Create account" : "Sign in";
  $("#login-toggle-text").textContent = signup ? "Already have an account?" : "New here?";
  $("#login-toggle").textContent = signup ? "Sign in" : "Create an account";
...
async function submitLogin() {
  ...
  btn.disabled = true;
  try {
    if (state.loginMode === "signup") await state.cloud.signUpEmail(email, password);
    else await state.cloud.signInEmail(email, password);
    closeModals();
    toast("Signed in.");
  } catch (e) {
...
async function loginGoogle() {
  try {
    await state.cloud.signInGoogle();
    closeModals();
    toast("Signed in.");
  } catch (e) {
...
async function saveCurrentResult() {
  if (!state.lastTables.length) return;
  if (!state.user) return openLogin("signin");
  const btn = $("#save-cloud");
  btn.disabled = true;
  try {
    const totalRows = state.lastTables.reduce((s, t) => s + t.rows.length, 0);
    await state.cloud.saveConversion({
      filename: state.filename,
      label: state.lastLabel,
      columns: state.lastTables[0] ? state.lastTables[0].columns : [],
      rowCount: totalRows,
      tableCount: state.lastTables.length,
...
async function openHistory() {
  ...
  list.innerHTML = "";
  list.appendChild(el("p", "history-empty", "Loading…"));
  $("#history-modal").hidden = false;
  try {
    const items = await state.cloud.listConversions();
    list.innerHTML = "";
    if (!items.length) {
      list.appendChild(el("p", "history-empty", "No saved conversions yet. Extract a statement and click “☁ Save”."));
      return;
    }
    items.forEach((it) => list.appendChild(historyRow(it)));
  } catch (e) {
    list.innerHTML = "";
    list.appendChild(el("p", "history-empty", "Could not load history: " + (e.message || e)));
  }
}
...
function historyRow(it) {
  ...
  meta.appendChild(el("div", "h-name", it.filename));
  const when = it.createdAt ? it.createdAt.toLocaleString() : "just now";
  meta.appendChild(el("div", "h-sub", `${when} · ${it.rowCount} rows · ${(it.columns || []).join(", ").slice(0, 60)}`));
  const actions = el("div", "history-actions");
  const base = (it.filename || "statement").replace(/\.pdf$/i, "");
  const csv = el("button", "btn", "CSV");
  csv.addEventListener("click", () => { try { downloadCsv(it.tables, base + ".csv"); } catch (e) { toast("Export failed", true); } });
  const xlsx = el("button", "btn primary", "Excel");
  xlsx.addEventListener("click", () => { try { downloadXlsx(it.tables, base + ".xlsx"); } catch (e) { toast("Export failed", true); } });
  const del = el("button", "btn subtle", "🗑");
  del.title = "Delete";
  del.addEventListener("click", async () => {
    if (!window.confirm("Delete this saved conversion?")) return;
    try { await state.cloud.deleteConversion(it.id); row.remove(); toast("Deleted."); }
    catch (e) { toast("Could not delete: " + (e.message || e), true); }
  });
  actions.appendChild(csv); actions.appendChild(xlsx); actions.appendChild(del);
  row.appendChild(meta); row.appendChild(actions);
  return row;
}

function initCloudUI() {
  $("#signin-btn").addEventListener("click", () => openLogin("signin"));
  $("#signout-btn").addEventListener("click", async () => { await state.cloud.signOut(); toast("Signed out."); });
  $("#history-btn").addEventListener("click", openHistory);
  $("#save-cloud").addEventListener("click", saveCurrentResult);
  $("#login-submit").addEventListener("click", submitLogin);
  $("#login-google").addEventListener("click", loginGoogle);
  $("#login-toggle").addEventListener("click", () => setLoginMode(state.loginMode === "signup" ? "signin" : "signup"));
  ...
  // Bring up Firebase (no-op if unavailable); keep the converter fully usable.
  initCloud((user) => { state.user = user; updateAuthUI(); }).then((api) => {
    state.cloud = api;
    updateAuthUI();
  }).catch(() => { state.cloud = null; updateAuthUI(); });
}

/* -------------------------------- Boot --------------------------------- */
document.addEventListener("DOMContentLoaded", () => {
  initUpload(); initTabs(); initAuto(); initManual(); initExport(); initCloudUI();
});
